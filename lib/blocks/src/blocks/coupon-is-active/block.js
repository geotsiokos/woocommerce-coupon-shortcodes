...
import { registerBlockType, withSelect } from '@wordpress/blocks';
		...
		/*edit: withSelect(
			( select ) => {
				return {
					...
					coupon_codes: "codecoupon", //select( 'woocommerce-coupon-shortcodes/woocommerce-coupon-shortcodes-blocks' ).getCouponCodes(),
				};
			}
		)
						...
						<PanelBody title={ __( 'Block Attributes', 'woocommerce-coupon-shortcodes' ) } className="block-inspector">
							<PanelRow>
								<label htmlFor="block-groups" className="groups-inspector__label">
									{ __( 'Content will be shown to users that are members of these groups:', 'woocommerce-coupon-shortcodes' ) }
								</label>
							</PanelRow>
							<PanelRow>
								<TextControl
